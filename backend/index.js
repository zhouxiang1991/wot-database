...
app.post('/tank', async (req, res) => {
  ...
  const a = await tanks.find(req.body).toArray()
...
app.post('/module', async (req, res) => {
  ...
  const a = await modules.find(req.body).toArray()
...
//   const nationLinkList = await page.$$eval(nationLink, (e) => Array.from(e).filter((item) => item.innerText !== '首页')
//     .map((item) => item.href))
